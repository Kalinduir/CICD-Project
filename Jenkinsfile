pipeline {
    agent any
     ...
     environment {
            DOCKER_IMAGE = "kalindudev/spring-boot-app:latest"
            DOCKER_CREDENTIALS_ID = "dockerhub-credentials"
            SSH_CREDENTIALS_ID = "learnfi-prod-server"
            SSH_TARGET = "ubuntu@34.203.124.37"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'dev', url: 'https://gitlab.com/kalinduir/cicd-sample-project.git', credentialsId: 'kalinducicd'
            }
        }
